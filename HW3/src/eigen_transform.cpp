...
Eigen::Vector3d rad2deg(Eigen::Vector3d radians) //function: returntype name (parameter)
{
    // Implement your code here
    radians = (180*radians)/M_PI; //radian converts to degree,degrees = radian*180/PI()
    return radians;
}
...
{
    // Implement your code here
    degrees=(M_PI*degrees)/180; //degree converts to radian, radian = degree * PI()/180
    return degrees;
}
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
       ...
       //std::cout << Q_tmp.coeffs().transpose() << std::endl;
        ...
        QQ.w()=0; //real part
        QQ.vec()=world_point;//imagine axis
        Eigen::Quaterniond RotatedQQ= Q_tmp*QQ*Q_tmp.inverse(); //quaternion: q*vector*q^-1
        world_point=RotatedQQ.vec();
    	
   
        // euler_tmp_deg.z()=s*90;
